...
exports.seedDemoProducts = exports.seedDemoUsers = void 0;
...
const seedDemoProducts = async () => {
    const vendorUser = await User_1.UserModel.findOne({ email: 'vendor@marketplace.com' }).lean();
    if (!vendorUser) {
        return;
    }
    let organization = await Organization_1.OrganizationModel.findOne({ name: 'Northstar Supply Co.' });
    if (!organization) {
        organization = await Organization_1.OrganizationModel.create({
    ...
    if (!vendorUser.organizationId && organization?._id) {
        await User_1.UserModel.findByIdAndUpdate(vendorUser._id, { organizationId: organization._id });
    }
    for (const product of demoProducts) {
        const slug = product.name.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '');
        const existingProduct = await Product_1.ProductModel.findOne({ slug });
        if (existingProduct) {
            continue;
        }
        const imageList = Array.isArray(product.images) && product.images.length > 0 ? product.images : [product.images?.[0] ?? ''];
        await Product_1.ProductModel.create({
...
exports.seedDemoProducts = seedDemoProducts;
