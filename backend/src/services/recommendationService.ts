...
export class RecommendationService {
  ...
  public rankProductsForUser(products: Array<Record<string, any>>, context: RecommendationContext = {}): RecommendationResult[] {
  ...
  public async generateForUser(userId: string): Promise<RecommendationResult[]> {
    const topProducts = await ProductModel.find({ isPublished: true })
    ...
    try {
      ...
      const parsed = JSON.parse(content) as { recommendations?: RecommendationResult[] };
  ...
  public async analyzeReviewSentiment(text: string): Promise<SentimentResult> {
